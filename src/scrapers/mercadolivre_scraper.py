...
class MercadoLivreScraper(BaseScraper):
    """Scraper para produtos do Mercado Livre Brasil."""
    ...
    def search_products(self, query: str, category: str = None) -> list[Product]:
        ...
        try:
            ...
            search_url = f"{self.base_url}/jm/search?as_word={query}"
            logger.info(f"Buscando '{query}' no Mercado Livre...")
            ...
            product_elements = soup.find_all("div", class_="ui-search-result__content")
    ...
    def _parse_product_element(self, element) -> Optional[Product]:
        ...
        try:
            # Extrair título
            title_elem = element.find("h2", class_="ui-search-item__title")
            if not title_elem:
                return None
            ...
            # Extrair preço
            price_elem = element.find("span", class_="price-tag-fraction")
            if not price_elem:
                return None
            ...
            product_link = element.find("a", class_="ui-search-link")
            product_url = product_link["href"] if product_link else ""

            # Extrair brand e model
            brand, model = self._parse_brand_model(title)

            # Verificar disponibilidade (Mercado Livre não mostra bem)
            in_stock = True
            ...
            product = Product(
                ...
                brand=brand,
                model=model,
                specifications={"source": "Mercado Livre"},
            )
    ...
    def _parse_price(price_text: str) -> Optional[float]:
        ...
        try:
            ...
            # Mercado Livre usa ponto para milhares e vírgula para decimais
...
if __name__ == "__main__":
    ...
    results = scraper.search_products("gpu rtx 4060")
