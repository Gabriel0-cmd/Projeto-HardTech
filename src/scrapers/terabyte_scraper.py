...
class TerabyteScraper(BaseScraper):
    """Scraper para produtos da Terabyte Shop."""

    def __init__(self):
        """Inicializa o scraper da Terabyte."""
        super().__init__(
            store_name="Terabyte",
    ...
    def search_products(self, query: str, category: str = None) -> list[Product]:
        ...
        try:
            # URL de busca da Terabyte
            search_url = f"{self.base_url}/busca.php?termo={query}"
            logger.info(f"Buscando '{query}' na Terabyte...")
    ...
    def _parse_product_element(self, element) -> Optional[Product]:
        ...
        try:
            ...
            product_link = element.find("a", class_="link")
            product_url = product_link["href"] if product_link else ""

            # Extrair brand e model
            brand, model = self._parse_brand_model(title)

            # Verificar estoque (Terabyte usa classes específicas)
            stock_elem = element.find("span", class_="stock")
            in_stock = True
            if stock_elem and "fora" in stock_elem.text.lower():
                in_stock = False
            ...
            product = Product(
                ...
                brand=brand,
                model=model,
                specifications={"source": "Terabyte"},
            )
...
if __name__ == "__main__":
    ...
    results = scraper.search_products("processador ryzen 5")
